...
This Terraform configuration deploys an EKS cluster with Crossplane and supports two deployment modes:

## Deployment Modes

### 1. **Create New VPC Mode (Default)**
- Creates a new VPC with 3 Availability Zones
- Creates 3 Private Subnets and 3 Public Subnets with appropriate Kubernetes tags
- Creates Internet Gateway for Public Subnets and NAT Gateway for Private Subnets
- Creates EKS Cluster Control plane with one managed node group

### 2. **Existing VPC Mode**
- Deploys EKS cluster into your existing VPC infrastructure
- Uses your existing private and public subnets
- Maintains your current networking configuration (NAT gateways, route tables, etc.)
- Ideal for organizations with standardized VPC configurations

## Components Deployed

Both modes deploy:
- EKS Cluster Control plane with managed node group (SPOT instances by default)
- Crossplane Add-on to EKS Cluster
- Upbound AWS Provider for Crossplane (default)
- AWS Provider for Crossplane (optional)
...
#### Option A: Deploy with New VPC (Default)
Use this option if you want Terraform to create all networking infrastructure for you.

**Required Variables:**
- `region` (default: us-east-1)
- `name` (default: crossplane-blueprints)

**Optional Variables:**
- `vpc_cidr` (default: 10.0.0.0/16)
- `cluster_version` (default: 1.29)
- `capacity_type` (default: SPOT)

#### Option B: Deploy with Existing VPC
Use this option if you have an existing VPC and want to deploy EKS into it.

**Required Variables:**
- `region`
- `name`
- `create_vpc = false`
- `vpc_id` - Your existing VPC ID (e.g., vpc-1234567890abcdef0)
- `private_subnet_ids` - List of at least 2 private subnet IDs in different AZs
...
**Important Subnet Requirements for Existing VPC:**

Your subnets must be tagged for Kubernetes load balancer discovery:
...
To add tags to your subnets, use:
```bash
# Tag private subnets
...
export TF_VAR_vpc_id=vpc-1234567890abcdef0
export TF_VAR_private_subnet_ids='["subnet-111111111111","subnet-222222222222"]'
export TF_VAR_public_subnet_ids='["subnet-333333333333","subnet-444444444444"]'
terraform plan
```

Or create a `terraform.tfvars` file (see `examples/` directory for templates):
```hcl
# terraform.tfvars for existing VPC
region             = "us-east-1"
name               = "my-eks-cluster"
create_vpc         = false
vpc_id             = "vpc-1234567890abcdef0"
private_subnet_ids = ["subnet-111111111111", "subnet-222222222222"]
public_subnet_ids  = ["subnet-333333333333", "subnet-444444444444"]
